...
import {CanBookingStart} from '../service/Api';
...
export default class Pending extends Component {
  ...
  apicall = () => {
    const url = 'http://astrourjaa.com:5050/api/' + 'schedule_bookings_astro';
  ...
  requestHandler = async (what, item) => {
    //this.props.routes.params.callbackfunction();
    // var date1 = moment();
    // var date2 = moment(item.schedule_date_time);
    // var days = date2.diff(date1, 'minute');
    // if (days > 0) {
    //   alert(`Please wait until ${item.schedule_date_time}`);
    //   return;
    // }

    if (what == '1') {
      let data = await CanBookingStart({
        booking_id: item.id,
      });
      //  alert(JSON.stringify(data));
      if (data.start_or_end == '0') {
        alert(data.message);
        return;
      }
    }
    ...
    socket.emit('accept_reject_schedule_booking', {
      booking_id: item.id,
      what: what,
    });
    if (what == '1') {
          ...
          <View
            ...
            <View
              ...
              <Text
                ...
                Booking Date : {itemData.item.created_at}
              </Text>
            </View>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <View style={{marginTop: 12}}>
                ...
                <Text
                  ...
                  Wallet: ₹ {itemData.item.user.wallet}
                </Text>

                <View style={{flexDirection: 'row'}}>
                  <TouchableOpacity
                    style={styles.bv_touch_accept}
                    onPress={() => this.requestHandler('1', itemData.item)}>
                    <Text style={styles.bv_t4}>START</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.bv_touch_decline}
                    onPress={() => this.requestHandler('2', itemData.item)}>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <View style={{marginLeft: 10}}>
            ...
            <Text
              ...
              {moment(data.schedule_date_time).format('DD MMM YY, HH:mm:a')}{' '}
            </Text>
