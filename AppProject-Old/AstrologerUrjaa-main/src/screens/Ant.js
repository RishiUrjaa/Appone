...
<FlatList
  ...
  renderItem={({item}) => {
    ...
    return (
         <TouchableOpacity activeOpacity = {0.99} onPress= {()=>console.log('s')}>
      <Image
...
{broadCastJoiner.length !== 0 && (
  <View
    ...
    {broadCastJoiner.map(item => {
      return (
        <Image
          source={
            item !== '' ? {uri: item} : require('../assets/add.png')
          }
          style={{
            width: 40,
            height: 40,
