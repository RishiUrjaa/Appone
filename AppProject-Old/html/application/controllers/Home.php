...
class Home extends CI_Controller
{
    ...
    public function delete_account()
    {
        ...
        // print_r("dd"); die;
        $template['page'] = 'home/delete_account';
        $template['page_title'] = "Astrostory";
...
if($_POST) {
    ...
    if (!empty($_FILES['image'])) {
        $target_path = "admin/admin/uploads/support/";
        $target_dir = "admin/admin/uploads/support/";
    ...
    $result =  $this->db->insert('delete_account', $data_doc);
    $this->session->set_flashdata('message', array('message' => 'Thanks you.', 'class' => 'success'));
    redirect(base_url('home/thanku'));
}


        //$template['terms'] = $this->db->query("select terms_condition from settings where id = 1")->row();
        $this->load->view('home/delete_account');
    }




    public function astrologer_change_password($value = '')
    {

        if ($_POST) {
              $d['password'] = md5($this->input->post("password"));
              $d['random_password'] = $this->input->post("password");

            $this->db->where('id', $value);
            $this->db->update('astrologers', $d);
            redirect(base_url('home/thanku_password'));
        }
        $template['page'] = 'home/create_password';
        $template['page_title'] = "login";
        $this->load->view( "home/create_password");
    ...
    {
       $this->db->where('phone',$_POST['mobile_number']);
    //   $this->db->where_in('status',array(1, 0, 2));
    ...
    public function pki()
    {
        echo '88BE50AFABC0B4CFA7CB8CB057975CF2BCD02E8A08ACA71857F844C90A72E942
sectigo.com
735074610';
    }
    ...
    public function astrologer_terms()
    {

        $template['page'] = 'home/astrologer_terms';
        $template['page_title'] = "Astrokul";
        //$template['terms'] = $this->db->query("select terms_condition from settings where id = 1")->row();
        $this->load->view('home/astrologer_terms');
    }
    ...
    public function thanku_password()
    {
        $this->load->view('home/thanku_password');
    }

    function getState(){
        $country_id =  $_POST['ide'];

        $query1 = $this->db->query("select * from states where country_id = $country_id")->result();
    ...
    public function astrologer_registration()
    {
        ...
        $template['countries'] = $this->db->query("SELECT * FROM `countries` ORDER BY `id` ASC")->result();
        ...
        if ($_POST) {
            ...
            if (count($check1) > 0) {
                ...
                if (!empty($_FILES['aadhar_card_front_image'])) {
                    $target_path = "admin/admin_ci/uploads/astrologers/";
                    $target_dir = "admin/admin_ci/uploads/astrologers/";
                ...
                // print_r($display_aadhar_card_front_image); die;
                if (!empty($_FILES['aadhar_card_back_image'])) {
                    $target_path = "admin/admin_ci/uploads/astrologers/";
                    $target_dir = "admin/admin_ci/uploads/astrologers/";
                    $target_file = $target_dir . basename($_FILES["aadhar_card_back_image"]["name"]);
                    $imagename = basename($_FILES["aadhar_card_back_image"]["name"]);
                    $extension = substr(strrchr($_FILES['aadhar_card_back_image']['name'], '.'), 1);
                    $actual_image_name = time() . "adh1." . $extension;
                    move_uploaded_file($_FILES["aadhar_card_back_image"]["tmp_name"], $target_path . $actual_image_name);
                    if (!empty($actual_image_name) && !empty($extension)) {
                        $display_aadhar_card_back_image = $actual_image_name;
                    } else {
                        $display_aadhar_card_back_image = 'default.png';
                    }
                } else {
                    $display_aadhar_card_back_image = 'default.png';
                }
                ...
                // if (!empty($_FILES['academic_certificate'])) {
                //     $target_path = "admin/admin/uploads/astrologers/";
                //     $target_dir = "admin/admin/uploads/astrologers/";
                //     $target_file = $target_dir . basename($_FILES["academic_certificate"]["name"]);
                //     $imagename = basename($_FILES["academic_certificate"]["name"]);
                //     $extension = substr(strrchr($_FILES['academic_certificate']['name'], '.'), 1);
                //     $actual_image_name = time() . "pan." . $extension;
                //     move_uploaded_file($_FILES["academic_certificate"]["tmp_name"], $target_path . $actual_image_name);
                //     if (!empty($actual_image_name) && !empty($extension)) {
                //         $display_academic_certificate = $actual_image_name;
                //     } else {
                //         $display_academic_certificate = 'default.png';
                //     }
                // } else {
                //     $display_academic_certificate = 'default.png';
                // }
                ...
                //     $extension2 = substr(strrchr($_FILES['certificate']['name'], '.'), 1);
                //     $actual_certificate_name_certificate = time() . "profile." . $extension2;
                //     move_uploaded_file($_FILES["certificate"]["tmp_name"], $target_path . $actual_certificate_name_certificate);
                //     if (!empty($actual_certificate_name_certificate) && !empty($extension2)) {
                //         $certificate = $actual_certificate_name_certificate;
                //     } else {
                //         $certificate = 'default.png';
                //     }
                // } else {
                //     $certificate = 'default.png';
                // }
                ...
                $chat_status= $permissions ? in_array('chat',$permissions) ? 1 : 0 : 0;
                $video_status=  $permissions ? in_array('video',$permissions) ? 1 : 0: 0;
                $audio_status= $permissions ? in_array('audio',$permissions) ? 1 : 0: 0;
                $booking= $permissions ? in_array('booking',$permissions) ? 1 : 0: 0;
                // $puja_booking= $permissions ? in_array('puja_booking',$permissions) ? 1 : 0: 0;
                // $can_specific_report =  $permissions ? in_array('specificreport',$permissions) ? 1 : 0: 0;
                // $can_take_horoscope= $permissions ? in_array('report',$permissions) ? 1 : 0: 0;
                // $can_take_broadcast= $permissions ? in_array('broadcast',$permissions) ? 1 : 0: 0;
                // $can_take_askquestion= $permissions ? in_array('askquestion',$permissions) ? 1 : 0: 0;
                $arr=[
                    'online_consult'=>0,
                    'chat_status'=>0,
                    'video_status'=>0,
                ...
                $service_offered2 =   $this->input->post("specialization") ; 
                foreach ($service_offered2 as $key33 ) {
                  
                  $ddd =   $this->db->query("SELECT * FROM `master_specialization` WHERE id = $key33")->row();
                ...
                for ($i = 0; $i < $count; $i++) {
                    // print_r( $files['certificate']['name'][$i]);die;

                    $target_path = "admin/admin_ci/uploads/astrologers/";
                    $target_dir = "admin/admin_ci/uploads/astrologers/";
                    $target_file = $target_dir . basename($files['certificate']['name'][$i]);
                    $imagename = basename($files['certificate']['name'][$i]);
                    $extension2 = substr(strrchr($files['certificate']['name'][$i], '.'), 1);
                    $actual_image_name_certificate = time() .$i. "certificate." . $extension2;
                    move_uploaded_file($files['certificate']['tmp_name'][$i], $target_path . $actual_image_name_certificate);
                    if (!empty($actual_image_name_certificate) && !empty($extension2)) {
                        $certificate[] = $actual_image_name_certificate;
                    } else {
                        $certificate[] = 'default.png';
                    }
                ...
                $count1 = count($_FILES['academic_certificate']['name']);
                for ($ii = 0; $ii < $count1; $ii++) {
                    // print_r( $files['certificate']['name'][$i]);die;

                    $target_path = "admin/admin_ci/uploads/astrologers/";
                    $target_dir = "admin/admin_ci/uploads/astrologers/";
                    $target_file = $target_dir . basename($files['academic_certificate']['name'][$ii]);
                    $imagename = basename($files['academic_certificate']['name'][$ii]);
                    $extension22 = substr(strrchr($files['academic_certificate']['name'][$ii], '.'), 1);
                    $actual_image_name_academic_certificate = time() .$ii. "academic_certificate." . $extension22;
                    move_uploaded_file($files['academic_certificate']['tmp_name'][$ii], $target_path . $actual_image_name_academic_certificate);
                    if (!empty($actual_image_name_academic_certificate) && !empty($extension22)) {
                        $academic_certificate[] = $actual_image_name_academic_certificate;
                    } else {
                        $academic_certificate[] = 'default.png';
                    }

                }
                $tags1 = implode(', ', $academic_certificate);

                	// print_r($tags);die;
                // foreach($certificates as $img_clinic) {
                //     $clinic11 = array(
                //         'doctor_id' => $this->input->post('docter_id'),
                //         'image' => $img_clinic,
                //     );
                //     $clinic_img = $this->Doctordetail_model->insert_data('doctor_gallery', $clinic11);
                // }
                ...
                $price_per_mint_audio = floatval($this->input->post('price_per_mint_audio'));
                ...
                $data_doc = array(
                    ...
                    'educational_background' => $this->input->post('education_qualification'),
                    'astrological_qalifications' => $this->input->post('astrological_qalifications'),
                    ...
                    'online_status' => 0,
                    'expertise' => $this->input->post('expertise') ? implode(',',$this->input->post('expertise')) : '',
                    'pan_card_image' => $display_pan_card_image,
                    'aadhar_card_back_image' => $display_aadhar_card_back_image,
                    'academic_certificate' => $tags1,
                    ...
                    'price_per_mint_chat_usd' => floatval($this->input->post('price_per_mint_chat_usd')),
                    'price_per_mint_video_usd' => floatval($this->input->post('price_per_mint_video_usd')),
                    'price_per_mint_audio_usd' => floatval($this->input->post('price_per_mint_audio_usd')),
                    'videototal_minute' => $this->input->post('videototal_minute'),
                    'chattotal_minute' => $this->input->post('chattotal_minute'),
                    'audiototal_minute' => $this->input->post('audiototal_minute'),
                    'education_qualification' => $this->input->post('education_qualification'),
            
                    // 'fifteen_min_price' => $this->input->post('fifteen_min_price'),
                    // 'thirty_min_price' => $this->input->post('thirty_min_price'),
                    // 'fortyfive_min_price' => $this->input->post('fortyfive_min_price'),
                    // 'sixty_min_price' => $this->input->post('sixty_min_price'),
